...
class PolicyNet(torch.nn.Module):
    ...
    def get_action_and_logp(self, x):
        ...
        :return:
        :exampole:
            action_prob = tensor([0.1000, 0.1000, 0.2000, 0.6000])
            action.item() : 2, logp : -1.6094379425048828
        """
...
def policyGradient(env, max_num_steps=1000, gamma=0.98, lr=0.01,
    ...
    :param gamma:
    :param lr:
    :param num_traj: number of trajectories {r^i} following the policy
    :param num_iter: episode
    ...
    # Initialize the value network parameter
    ...
    for it in range(num_iter):
        ...
        # Compute the MSE of the value network
        # policy gradient 및 policy gradient with reward-to-go 사용하는 경우 value network 불필요
